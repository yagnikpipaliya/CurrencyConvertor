...
const CurrencyConverter = () => {
  ...
  useEffect(() => {
    const fetchExchangeRates = async () => {
      try {
        ...
        // setAmount2((1 / rates[currency2]).toFixed(2));  //response.data.
      } catch (error) {
  ...
  return (
    <>
          ...
          <div className="w-full relative flex justify-between pb-0 top-0">
            <div className="relative w-full block">
              <div className="p-1 caret-[#8ab4f8] border-[2px] border-solid border-[#5f6368] rounded-md color-[#9aa0a6] overflow-hidden flex mb-5 hover:border-[#8ab4f8] hover:border-[2px] ">
                ...
                />
                {/* w-36  <input type="text" inputmode="numeric" className="text-black"/>   202128*/}
      ...
      {/* <label>
            Amount 1 ({currency1}):
            <input className="text-black" type="number" value={amount1} onChange={handleAmount1Change} placeholder={`Enter amount in ${currency1}`} />
          </label>

          <label>
            Currency 1:
            <select value={currency1} onChange={handleCurrency1Change} className="text-black">
              {exchangeRates &&
                Object.keys(exchangeRates).map((currency) => (
          ...
          <label>
            Amount 2 ({currency2}):
            <input className="text-black" type="number" value={amount2} onChange={handleAmount2Change} placeholder={`Enter amount in ${currency2}`} />
          </label>

          <label>
            Currency 2:
            <select value={currency2} onChange={handleCurrency2Change} className="text-black">
              {exchangeRates &&
                Object.keys(exchangeRates).map((currency) => (
                  <option key={currency} value={currency}>
                    {currency.toUpperCase()}
